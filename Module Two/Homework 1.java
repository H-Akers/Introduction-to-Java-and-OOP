            ...
            for (int i = 0; i < arg.length(); i++){
                ...
                if (Character.isLetter(arg.charAt(i))) {
                    for(int l = 0; l < newString.length(); l++){
                        if (newString.charAt(l) != arg.charAt(i)) 
                            temp = temp + newString.charAt(l);
                    }
                    newString = temp;
	...
	public String reverse(String arg) {
        ...
        if (this.myString == null || this.myString.isEmpty() || this.isSet == false) 
            ...
            for (int i = 0; i < arg.length(); i++) {
                ...
                if (Character.isLetter(arg.charAt(i))) {
                    for (int l = 0; l < newString.length(); l++) {
                        char l1 = newString.charAt(l);
                        char l2 = arg.charAt(i);
                        if(Character.toLowerCase(l1) == Character.toLowerCase(l2)) {

                        
                            if (Character.isLowerCase(l1)) 
                                temp = temp + Character.toUpperCase(l1);
                            
                            else 
                                temp = temp + Character.toLowerCase(l1);
	...
	public String filterLetters(char n, boolean more) {
		...
		{			
			if(more)
			{
				...
				{
					char l = myString.charAt(i);
					
					if(Character.isLetter(l))
					{
						if((l >= 'a' && l < Character.toLowerCase(n)) || (l >= 'A' && l < Character.toUpperCase(n)))
			...
			else
			{
				for(int i=0; i<this.myString.length(); ++i)
				{
					...
					if(Character.isLetter(l))
					{
						if((l > Character.toLowerCase(n) && l <= 'z') || (l > Character.toUpperCase(n) && l <= 'Z'))
							newString = newString + l;
					}
					else
						newString = newString + l;
